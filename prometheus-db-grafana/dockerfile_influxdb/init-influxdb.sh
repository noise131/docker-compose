...
if ( [ ! -z "$INIT_USERS" ] || [ ! -z "$INFLUXDB_DB" ] || [ "$(ls -A /docker-entrypoint-initdb.d 2> /dev/null)" ] ) && [ ! "$(ls -d "$META_DIR" 2>/dev/null)" ]; then
	...
	INIT_QUERY=""
	DURATION=${INFLUXDB_DURATION:-0s}
	SHARD_DURATION=${INFLUXDB_SHARD_DURATION:-7d}

	CREATE_DB_QUERY="CREATE DATABASE $INFLUXDB_DB WITH DURATION $DURATION SHARD DURATION $SHARD_DURATION"
